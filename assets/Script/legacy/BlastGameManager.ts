...
@ccclass
export default class BlastGameManager extends cc.Component {
    ...
    private validateBlueprints() {
        if (this.gameSettings.tileBlueprints.length == 0) {
            cc.error('BlastGameManager.validateBlueprints(): tileBlueprints is empty');
            return;
        }
    ...
    private checkExistingTurns() {
        cc.log('BlastGameManager.checkExistingTurns()');
        if (this.grid.haveExistingTurns(this.gameSettings.minBlastGroupSize)) {
            this.isProcessing = false;
            return;
        }

        cc.log('BlastGameManager.checkExistingTurns() there is no turns');
